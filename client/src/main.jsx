...
async function invitationApi(path, body) {
...
function AccountPill({ account }) { if(!account.enabled)return <span className="pill off">Paused</span>;if(account.passwordSet)return <span className="pill on">Active</span>;const expired=account.inviteExpiresAt&&new Date(account.inviteExpiresAt)<=new Date();const label=account.inviteError?'Email failed':expired?'Setup expired':'Setup pending';return <span className={`route-pill ${account.inviteError||expired?'error':'pending_verification'}`}>{label}</span>; }
...
function Dashboard({ onLogout }) {
  ...
  async function action(fn, message) {
    setError(''); setNotice('');
    try { const result=await fn(); await load(); setNotice(result?.warning||message); setTimeout(() => setNotice(''), 4000); }
...
function Accounts({ state, action }) {
  ...
  const [localPart, setLocalPart] = useState(''); const [domain, setDomain] = useState(availableDomains[0]?.name || ''); const [forwardTo, setForwardTo] = useState('');
  useEffect(() => { if (!availableDomains.some((item) => item.name === domain)) setDomain(availableDomains[0]?.name || ''); }, [state.domains, domain]);
  async function add(e) { e.preventDefault(); await action(() => api('/accounts', { method:'POST', body:JSON.stringify({localPart,domain,forwardTo}) }), 'Account added; setup and verification emails requested'); setLocalPart(''); setForwardTo(''); }
  ...
  return <><section className="panel form-panel"><div><h2>Add employee</h2><p>Sends a private password setup link and starts Cloudflare destination verification.</p></div><form className="inline-form account-form" onSubmit={add}><input required maxLength="64" pattern="[^@]+" placeholder="name" aria-label="Account name" value={localPart} onChange={e=>setLocalPart(e.target.value)} /><select required aria-label="Company domain" value={domain} onChange={e=>setDomain(e.target.value)} disabled={!availableDomains.length}><option value="" disabled>Select domain</option>{availableDomains.map(item=><option key={item.id} value={item.name}>@{item.name}</option>)}</select><input type="email" required maxLength="90" placeholder="Personal email" aria-label="Personal email" value={forwardTo} onChange={e=>setForwardTo(e.target.value)} /><button disabled={!availableDomains.length}>Invite employee</button></form></section>
  <section className="panel"><div className="panel-title"><h2>Employee accounts</h2><span>{state.accounts.length} total</span></div><div className="table accounts-table"><div className="tr head"><span>Address</span><span>Personal email</span><span>Routing</span><span>Account</span><span>Created</span><span /></div>{state.accounts.map(a=><div className="tr" key={a.id}><strong>{a.email}</strong><span className="muted">{a.forwardTo||'—'}</span><span title={a.routingError||''}><RoutingPill status={a.routingStatus}/></span><span title={a.inviteError||''}><AccountPill account={a}/></span><span className="muted">{formatDate(a.createdAt)}</span><span className="row-actions">{!a.forwardTo&&<button className="subtle" onClick={()=>setForwarding(a)}>Personal email</button>}{['error','expired'].includes(a.routingStatus)&&<button className="subtle" onClick={()=>action(()=>api(`/accounts/${a.id}/routing/retry`,{method:'POST'}),'Routing check restarted')}>Retry routing</button>}<button className="subtle" onClick={()=>action(()=>api(`/accounts/${a.id}/invite`,{method:'POST'}),a.passwordSet?'Password reset link sent':'Setup link resent')}>{a.passwordSet?'Reset link':'Resend setup'}</button><button className="subtle" onClick={()=>action(()=>api(`/accounts/${a.id}`,{method:'PATCH',body:JSON.stringify({enabled:!a.enabled})}), a.enabled?'Account paused':'Account enabled')}>{a.enabled?'Pause':'Enable'}</button><button className="danger" onClick={()=>confirm(`Delete ${a.email} and its Cloudflare route?`)&&action(()=>api(`/accounts/${a.id}`,{method:'DELETE'}),'Account and route deleted')}>Delete</button></span></div>)}{!state.accounts.length&&<div className="empty">No employee accounts yet.</div>}</div></section></>;
}
...
function SetPassword(){const [token]=useState(()=>new URLSearchParams(window.location.hash.slice(1)).get('token')||'');const [account,setAccount]=useState(null);const [password,setPassword]=useState('');const [confirmation,setConfirmation]=useState('');const [error,setError]=useState('');const [done,setDone]=useState(false);useEffect(()=>{invitationApi('/inspect',{token}).then(setAccount).catch(e=>setError(e.message));},[token]);async function submit(e){e.preventDefault();setError('');if(password!==confirmation)return setError('Passwords do not match');try{const result=await invitationApi('/complete',{token,password});setAccount({...account,email:result.email});setDone(true);window.history.replaceState(null,'','/set-password');}catch(e){setError(e.message)}}if(done)return <main className="login-shell"><section className="login-card setup-card"><div className="mark">MR</div><h1>Password saved</h1><p className="muted">Your credentials for <strong>{account.email}</strong> are ready. Use this address and password for both POP3 and SMTP.</p></section></main>;return <main className="login-shell"><section className="login-card setup-card"><div className="mark">MR</div><p className="eyebrow">EMPLOYEE SETUP</p><h1>Choose your password</h1>{account?<><p className="muted">Set the POP3 and SMTP password for <strong>{account.email}</strong>. This link expires {new Date(account.expiresAt).toLocaleString()}.</p><form onSubmit={submit}><label>Password<input autoFocus required type="password" maxLength="200" value={password} onChange={e=>setPassword(e.target.value)} /></label><label>Confirm password<input required type="password" maxLength="200" value={confirmation} onChange={e=>setConfirmation(e.target.value)} /></label>{error&&<p className="error">{error}</p>}<button>Save password</button></form></>:error?<p className="error">{error}</p>:<p className="muted">Checking your setup link…</p>}</section></main>}

function AdminApp(){const [auth,setAuth]=useState(null);useEffect(()=>{api('/me').then(()=>setAuth(true)).catch(()=>setAuth(false));},[]);if(auth===null)return <main className="loading">Loading…</main>;return auth?<Dashboard onLogout={()=>setAuth(false)}/>:<Login onLogin={()=>setAuth(true)}/>;}
function App(){return window.location.pathname==='/set-password'?<SetPassword/>:<AdminApp/>;}
